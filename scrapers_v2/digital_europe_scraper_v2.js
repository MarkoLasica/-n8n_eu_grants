// scrapers_v2/digital_europe_scraper_v2.js
const BaseScraperV2 = require('./base_scraper_v2');

class DigitalEuropeScraperV2 extends BaseScraperV2 {
  constructor(config) {
    super('digital_europe', config);
    this.baseUrl = 'https://ec.europa.eu/info/funding-tenders/opportunities/portal/screen/opportunities/calls-for-proposals';
    // frameworkProgramme=43152860 filters for Digital Europe Programme only
    this.params = '?order=DESC&pageNumber=1&pageSize=50&sortBy=startDate&isExactMatch=true&status=31094502&frameworkProgramme=43152860';
  }
...
if (require.main === module) {
  ...
  const scraper = new DigitalEuropeScraperV2(config);
  scraper.run().then(() => process.exit(0)).catch(() => process.exit(1));
}

module.exports = DigitalEuropeScraperV2;
