// scrapers_v2/horizon_europe_scraper_v2.js
const BaseScraperV2 = require('./base_scraper_v2');

class HorizonEuropeScraperV2 extends BaseScraperV2 {
  constructor(config) {
    super('horizon_europe', config);
    this.baseUrl = 'https://ec.europa.eu/info/funding-tenders/opportunities/portal/screen/opportunities/calls-for-proposals';
    // frameworkProgramme=43108390 filters for Horizon Europe only
...
if (require.main === module) {
  ...
  const scraper = new HorizonEuropeScraperV2(config);
  scraper.run().then(() => process.exit(0)).catch(() => process.exit(1));
}

module.exports = HorizonEuropeScraperV2;
